...
# 全局路径
#================================================================
readonly CONF_DIR="/etc/ghulogin"
readonly CONF_FILE="${CONF_DIR}/ghulogin.conf"
readonly LOG_FILE="/var/log/ghulogin.log"
readonly PID_FILE="/var/run/ghulogin.pid"
readonly INIT_SCRIPT="/etc/init.d/ghulogin"
...
rsa_encrypt_password() {
    ...
    # 将Base64公钥写入临时文件
    local pubkey_file="/tmp/ghulogin_pubkey.pem"
...
do_login() {
    ...
    if [ -z "${USERNAME:-}" ] || [ -z "${PASSWORD:-}" ]; then
        print_error "未配置用户名或密码，请先运行 ghulogin config"
        return 1
    fi
    ...
    # 获取302重定向页面
    local response
    response=$(curl -s -m 10 "http://www.google.cn/generate_204" 2>/dev/null)
    if [ -z "$response" ]; then
        print_error "无法获取认证页面"
        return 1
    fi

    # 提取登录页URL
    local login_page_url
    login_page_url=$(echo "$response" | awk -F "'" '{print $2}')
...
do_keepalive() {
    if ! load_config; then
        print_error "未找到配置文件 $CONF_FILE，请先运行 ghulogin config"
        return 1
    fi

    print_info "ghulogin 保活模式启动"
...
do_logs() {
    ...
    echo -e "${BOLD}=== ghulogin 日志 (最近50条) ===${NC}"
...
#================================================================
do_install() {
    print_info "安装 ghulogin 到系统..."

    # 复制脚本到 /usr/bin
    local script_path
    script_path=$(readlink -f "$0" 2>/dev/null || echo "$0")
    cp "$script_path" /usr/bin/ghulogin
    chmod +x /usr/bin/ghulogin
    print_success "已安装到 /usr/bin/ghulogin"
...
start_service() {
    procd_open_instance
    procd_set_param command /usr/bin/ghulogin keepalive
    procd_set_param respawn 3600 5 5
    procd_set_param stdout 1
    procd_set_param stderr 1
    procd_set_param pidfile /var/run/ghulogin.pid
    procd_close_instance
}

stop_service() {
    killall ghulogin 2>/dev/null
}
INITEOF
    chmod +x "$INIT_SCRIPT"
    ...
    echo ""
    print_info "安装完成! 接下来:"
    echo -e "  ${CYAN}1.${NC} 运行 ${BOLD}ghulogin config${NC} 配置账号"
    echo -e "  ${CYAN}2.${NC} 运行 ${BOLD}ghulogin keepalive${NC} 或 ${BOLD}/etc/init.d/ghulogin start${NC} 启动"
}

do_uninstall() {
    print_warn "卸载 ghulogin..."
...
do_service_ctrl() {
    local action="$1"
    if [ ! -x "$INIT_SCRIPT" ]; then
        print_error "服务未安装，请先运行 ghulogin install"
        return 1
    fi
    case "$action" in
...
#================================================================
show_help() {
    echo -e "${BOLD}ghulogin${NC} - 锐捷ePortal自动认证工具"
    echo ""
    echo -e "${BOLD}用法:${NC}"
    echo "  ghulogin              交互式菜单"
    echo "  ghulogin <命令>       执行指定命令"
    echo ""
    echo -e "${BOLD}命令:${NC}"
    ...
    echo -e "${BOLD}示例:${NC}"
    echo "  ghulogin              # 打开菜单"
    echo "  ghulogin login        # 立即登录"
    echo "  ghulogin status       # 查看状态"
}
